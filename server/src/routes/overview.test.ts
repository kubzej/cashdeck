...
test('reads a bounded selection trend scoped to the verified user', async () => {
  ...
  const response = await app.inject({ method: 'GET', url: `/api/overview/selection?type=category&id=${categoryId}&walletIds=${walletId}&dateFrom=2026-08-01&dateTo=2026-08-31&granularity=day`, headers: { authorization: 'Bearer test-token' } })
  expect(response.statusCode).toBe(200)
  expect(response.json()).toEqual({ previous: { amountCzk: -9_000 }, series: [{ date: '2026-08-01', amountCzk: -3_000 }] })
  expect(repository.getSelectionTrend).toHaveBeenCalledWith(userId, { type: 'category', id: categoryId, walletIds: [walletId], dateFrom: '2026-08-01', dateTo: '2026-08-31', granularity: 'day' })
  await app.close()
})
