...
export function createDefaultFeedFilters(): FeedFilterValue {
  ...
  return { walletIds: [], period: 'month', customDateFrom: currentMonth.dateFrom, customDateTo: currentMonth.dateTo, search: '' }
}
...
export function resolveFeedDateRange(value: FeedFilterValue): FeedDateRange {
  ...
  if (value.period === 'week') return getWeekRange(today)
  if (value.period === 'year') return { dateFrom: `${today.slice(0, 4)}-01-01`, dateTo: `${today.slice(0, 4)}-12-31` }
...
export function FeedFilters({ wallets, value, onChange }: { wallets: Wallet[]; value: FeedFilterValue; onChange: (value: FeedFilterValue) => void }) {
  ...
  return <section className="feed-filters" aria-label="Filtry transakcí">
    ...
    <div className="feed-filters__controls">
      ...
      <PeriodFilterDialog value={value} label={getPeriodLabel(value.period, dateRange)} onApply={(next) => onChange({ ...value, ...next })} />
    </div>
  </section>
}
...
function getCurrentMonthRange() { return getMonthRange(getPragueToday()) }
