...
import { createWallet, walletColorKeys, type WalletColorKey } from './api'
import './wallets.css'
...
export function WalletFormScreen({ onCancel, onCreated }: { onCancel: () => void; onCreated: () => void }) {
  const [values, setValues] = useState<WalletFormValues>({ name: '', colorKey: 'teal', openingBalanceCzk: '', openingBalanceDate: getPragueToday() })
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createWallet({ name: values.name.trim(), colorKey: values.colorKey, openingBalanceCzk, openingBalanceDate: values.openingBalanceDate })
      onCreated()
    } catch (error) {
  ...
  return (
    <section className="wallet-form-screen" aria-labelledby="new-wallet-title">
      <header className="wallet-form-header">
        ...
        <h1 id="new-wallet-title">Nová peněženka</h1>
      ...
      <form className="wallet-form" onSubmit={(event) => void handleSubmit(event)} noValidate>
        ...
        <Button type="submit" size="lg" className="wallet-form-submit" loading={isSubmitting}>Uložit peněženku</Button>
      </form>
    </section>
  )
}
